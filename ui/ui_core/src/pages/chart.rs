...
(function () {
  ...
  window.scChartInit = function (containerId, theme) {
    ...
    var entry = { chart: chart, series: series };
  ...
  window.scChartSetData = function (containerId, candles) {
    ...
    chart_fit(entry.chart);
  };
  function chart_fit(chart) {
...
pub fn Chart(server_url: String) -> Element {
    ...
    let selected_interval = interval_choice();
    let series_key = format!("{selected_exchange}:{selected_pair}:{selected_interval}");
    ...
    use_future(move || {
        ...
        async move {
            ...
            let _ = document::eval(&format!(
                "window.scChartInit('{CONTAINER_ID}', '{theme_str}')"
            ))
            .await;
    ...
    // exchange/pair/interval changes.
    {
        let exchange = selected_exchange.clone();
        let pair = selected_pair.clone();
        let interval = selected_interval.clone();
    ...
    // bar whose `time` was already pushed.
    {
        let key = series_key.clone();
        use_effect(move || {
            ...
            spawn(async move {
                if let Ok(json) = serde_json::to_string(&latest) {
                    let _ =
                        document::eval(&format!("window.scChartUpdate('{CONTAINER_ID}', {json})"))
                            .await;
                }
            });
        });
