...
fn build_state_without_redis() -> actix_web::web::Data<AppState> {
...
#[actix_web::test]
async fn ticker_endpoint_returns_400_without_redis() {
    let app = test::init_service(
        App::new()
            .configure(init_routes)
            .app_data(build_state_without_redis())
...
#[actix_web::test]
async fn ticker_endpoint_returns_redis_unavailable_message() {
    let app = test::init_service(
        App::new()
            .configure(init_routes)
            .app_data(build_state_without_redis())
